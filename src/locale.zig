...
pub const Locale = struct {
    ...
    pub fn validateFileAtPath(full_path: []const u8, allocator: std.mem.Allocator) ![]const u8 {
        var file = try File.create(allocator, full_path, null);
        defer file.destroy();
        try file.loadSource();
        try file.buildTree();
        return validateFile(file, allocator);
    }

    fn validateFile(file: *File, allocator: std.mem.Allocator) ![]const u8 {
...
test "Localization Ids" {
    ...
    const file = try parser_test.parseSource(input);
    defer file.destroy();
    const update = try Locale.validateFile(input, file.tree, std.testing.allocator);
